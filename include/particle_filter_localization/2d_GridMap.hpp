...
/**
 * @brief 粒子滤波 栅格地图库   Copyright (C) 2020 Wenhao Li <3062451557@qq.com>
 * @author wenhao li
 * @date 2020.8.22
 **/
...
public:
    ...
    void SaveMap ( const std::string& img_dir, const std::string& cfg_dir )
    {
        ...
        if(img.empty()) 
        {
            std::cout << " save map is empty!!" << std::endl;
            return; 
        }

        std::cout << " save map width: "<<img.cols<<" height: "<<img.rows << std::endl;
        cv::imshow("map", img);
        cv::waitKey();
    ...
    /**
     * @brief 根据参数加载图片
     * @details 读取地图mat图片 将其转换为占据栅格地图  
     * @param[in] init_x, init_y 地图原点位置 
     * @param[in] img_dir 地图路径  地图是一个图片 
     **/
    void loadMap ( const std::string& img_dir, const int& init_x, const int& init_y, const double& cell_size )
    {
        ...
        if(cv_map.type() == CV_8UC1)    // 如果为灰度图片
            cv_gray = cv_map;  
        else
            cv::cvtColor(cv_map, cv_gray, cv::COLOR_RGB2GRAY); // 地图转换为灰度图片

        cv::imshow("visualize", cv_gray);
        cv::waitKey();  

        //cv::flip(cv_gray, cv_gray, 1);          // >0: 沿y-轴翻转  水平翻转 , 0: 沿x-轴翻转, <0: x、y轴同时翻转
        
        size_width_ = cv_gray.cols;     // 列  x
        size_height_ = cv_gray.rows;     // 行  y 
        // 获取初值是为了通过地图的位置信息求取栅格地图的index
        init_x_ = init_x;
        init_y_ = init_y;
        cell_size_ = cell_size;
        // 用图片的尺寸初始化  栅格地图数组 map_bel_data_     
        map_bel_data_.resize ( size_height_, vector<double>(size_width_, 0.5) );           
        
        /* 为opencv图片显示相关 */
        m_one_.resize( size_height_, vector<double>(size_width_, 1) );                             // 设置为 1  
        m_show_.resize( size_height_, vector<double>(size_width_, 0.5) );

        ///@brief 遍历图片的每一行  将值转换为 概率值  赋值给 bel_data_ 
        for (size_t x=0; x<cv_gray.cols; x++)
        {
            ...
            {    
                ...
                // 应该说保存的图片包含不完全是占用状态的,但是这里只load了完全占据的部分 ??
                switch (pixel)
                {   // 赋值 (行, 列)
                    case 0: //占用      即黑色
                        map_bel_data_[y][x] = 1.0;     
                    break;
                        
                    case 254: //free   白色
                        map_bel_data_[y][x] = 0.0;
                    break;
                        
                    case 205: //unknown   
                        map_bel_data_[y][x] = 0.5;
                    break;
                } //switch
            }// for y
        }// for x
    ...
    double minX() const 
    {
        return -init_x_ * cell_size_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    double maxX() const 
    {
        return ( size_width_ - init_x_ ) * cell_size_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    double minY() const 
    {
        return -init_y_ * cell_size_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    double maxY() const 
    {
        return ( size_height_ - init_y_ ) * cell_size_;
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    int const& GetInitX() const 
    {
        return init_x_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    int const& GetInitY() const
    {
        return init_y_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    double const& GetCellsize() const
    {
        return cell_size_;
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    vector<vector<double>> const& GetMapBelData() const
    {
        return map_bel_data_;
    }
